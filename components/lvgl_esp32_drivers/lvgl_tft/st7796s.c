...
void st7796s_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    // ESP_LOGI(TAG, "Flush colums %d to %d:", area->x1, area->x2);

    /*Column addresses*/
    st7796s_send_cmd(0x2A);                    // st7796s CASET
    ...
    /*Page addresses*/                       // st7796s RASET
    // ESP_LOGI(TAG, "Flush rows %d to %d:", area->y1, area->y2);
    ...
    // ESP_LOGI(TAG, "SPI send color map size: %d", size);

    st7796s_send_color((void *)color_map, size * 2);
    // ESP_LOGI(TAG, "Done flush");
}
...
static void st7796s_send_data(void *data, uint16_t length)
{
    // ESP_LOGI(TAG, "SPI send data %d bytes", length);
    disp_wait_for_pending_transactions();
    gpio_set_level(ST7796S_DC, 1); /*Data mode*/
    disp_spi_send_data(data, length);
    // ESP_LOGI(TAG, "SPI data sent");
}

static void st7796s_send_color(void *data, uint16_t length)
{
    // ESP_LOGI(TAG, "Flush data length %d", length);
    disp_wait_for_pending_transactions();
    gpio_set_level(ST7796S_DC, 1); /*Data mode*/
